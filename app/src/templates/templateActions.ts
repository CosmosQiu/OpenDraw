...
export function saveSelectionAsTemplate(
  ...
  const nodeShapes = selectedShapes.filter(
    (s): s is NodeShape => s.type === "node",
  );
...
export function stampTemplate(
  ...
  editor.run(() => {
    for (const tNode of template.nodes) {
      const shapeId = createNodeShapeAtPoint(editor, {
        type: tNode.nodeType.type,
        x: position.x + tNode.relativeX,
        y: position.y + tNode.relativeY,
        center: false,
        select: false,
        props: tNode.nodeType as unknown as Record<string, unknown>,
      });
