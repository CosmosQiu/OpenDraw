...
export class ConnectionShapeUtil extends ShapeUtil<ConnectionShape> {
  ...
  onHandleDrag(
    ...
    const existingConnectionOnTarget =
      target?.existingConnections.find(
    ...
    if (isCreatingShape && draggingTerminal === "end") {
      ...
      onCanvasNodePickerState.set(this.editor, {
        ...
        onPick: (nodeType, terminalInPageSpace) => {
          const newNodeId = createNodeShapeAtPoint(this.editor, {
            type: nodeType.type,
            x: terminalInPageSpace.x,
            y: terminalInPageSpace.y,
            center: false,
            select: true,
            props: nodeType as unknown as Record<string, unknown>,
          });
...
function ConnectionShapeComponent({
  connection,
}: {
  connection: ConnectionShape;
}) {
  ...
  const connectionColor = useValue(
    ...
    () => {
      ...
      const dataType = getPortDataType(
        editor,
        bindings.start.toId,
        bindings.start.props.portId,
      );
...
function ConnectionCenterHandle({
  ...
  return (
    ...
    >
      ...
      />
      <circle
        className="ConnectionCenterHandle-ring"
        r={CONNECTION_CENTER_HANDLE_SIZE_PX / 2}
      />
      <path
