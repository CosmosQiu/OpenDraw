...
export function createOrUpdateConnectionBinding(
  ...
  if (props.terminal === "end" && order == null) {
    const existingBindingsToTarget = editor
      ...
      .filter(
        (b) => b.props.terminal === "end" && b.props.portId === props.portId,
      );
    order =
      existingBindingsToTarget.reduce(
        (max, b) => Math.max(max, b.props.order ?? 0),
        0,
      ) + 1;
  }
  ...
  if (existing) {
    editor.updateBinding({
      ...
      props: {
        ...propsWithOrder,
        order: existing.props.order ?? propsWithOrder.order,
      },
    });
  } else {
