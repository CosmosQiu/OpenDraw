...
function findPipelineRegions(editor: Editor): PipelineRegion[] {
  ...
  return Array.from(
    new Set(regionsByShape.values()),
    (nodeIds): PipelineRegion => {
...
export function PipelineRegions() {
  ...
  const regions = useValue("regions", () => findPipelineRegions(editor), [
    editor,
  ]);

  return regions.map((region, i) => (
    <PipelineRegionComponent key={i} region={region} />
  ));
}
...
function PipelineRegionComponent({ region }: { region: PipelineRegion }) {
  ...
  return (
    <div
      className={classNames("PipelineRegion", {
        PipelineRegion_executing: isExecuting,
      })}
      ref={ref}
    >
      <button
